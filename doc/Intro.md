...
この問題を解決する１つの手段としてCSPがあります。これは並行プロセスを記述する為に作られたものですが、並行性や内部遷移についての演算を抜いても十分使える優れものです。

PrimalSpecはCSPから私のような原始人にはとても使いこなせない並行性と内部遷移についての高度な演算子を取り除いたものになっています。(他にも✓ がないといった差異もあります)
...
PrimeSpecでは、b=aだった場合は内部選択にせず、実行時エラーを出すようにしています。
